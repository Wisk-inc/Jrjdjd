...
# Keep the cell running after launch, printing a heartbeat and — the part that
# matters — the new tunnel address whenever cloudflared rotates it. Set to False
# to have the cell finish immediately as it used to.
KEEP_ALIVE   = True
...
# ---------------------------------------------------------------------------
# A whole song is a realistic upload and a bad idea: pitch tracking alone runs
# at roughly 3x realtime, so four minutes of audio is over a minute of CPU work
# before the model is even called. Clips are trimmed to this and the caller is
# told, rather than being left to wonder why nothing came back.
MAX_SECONDS = 60.0
...
def sniff_format(raw):
    ...
    libsndfile reports "Format not recognised" next to a BytesIO object address,
    which tells a person nothing about the file they just picked.
    """
    ...
    if len(raw) > 1 and raw[0] == 0xFF and (raw[1] & 0xE0) == 0xE0: return "MP3"
    if raw[4:8] == b"ftyp": return "MP4/M4A/AAC"
    if raw[:4] == b"\x1aE\xdf\xa3": return "WebM/Matroska"
    if raw[:4] == b"FORM": return "AIFF"
    if raw[:4] == b"caff": return "CAF"
    return "unknown"


def decode_audio(raw):
    """Bytes -> (float32 samples, sample rate), trying more than one decoder.

    libsndfile handles WAV, FLAC, OGG and MP3, but not the MPEG-4 family — and
    .m4a is what an iPhone voice memo and most Android recorders produce, which
    makes it the single most likely file for someone cloning their own voice.
    librosa/audioread covers it when a system codec exists.
    """
    errors = []
    try:
        ...
        errors.append("soundfile: %s" % str(e)[:80])
    ...
    try:
        ...
        errors.append("librosa/audioread: %s" % str(e)[:80])

    fmt = sniff_format(raw)
    # The decoder errors go to the log, not to the person: libsndfile's message
    # is "Format not recognised" next to a BytesIO object address, which reads
    # like a crash and says nothing about the file they picked.
    log("decode failed (%s): %s" % (fmt, "; ".join(errors)))
    raise ValueError("This build cannot decode %s. Re-export the clip as WAV, FLAC, "
                     "OGG or MP3 — all four work. On a phone, a voice memo is usually "
                     ".m4a, which is the case that lands here most often." % fmt)


def load_audio(raw, target_sr=None, max_seconds=MAX_SECONDS, label="clip"):
    """Bytes -> mono float32 at target_sr, trimmed and sanity-checked."""
    import librosa
    target_sr = target_sr or SAMPLE_RATE
    if not raw:
        raise ValueError("The %s was empty — no bytes arrived." % label)

    wav, sr = decode_audio(raw)
    if sr != target_sr:
        wav = librosa.resample(wav, orig_sr=sr, target_sr=target_sr)

    trimmed = 0.0
    if max_seconds and len(wav) > int(max_seconds * target_sr):
        trimmed = len(wav) / float(target_sr)
        wav = wav[:int(max_seconds * target_sr)]

    peak = float(np.max(np.abs(wav))) if wav.size else 0.0
    if wav.size < target_sr // 10:
        raise ValueError("The %s is only %.2fs long. Give it at least a second of "
                         "audio to work with." % (label, wav.size / float(target_sr)))
    if peak < 1e-4:
        raise ValueError("The %s is silent (peak %.2g). Check the right file was "
                         "picked and that it is not a muted export." % (label, peak))
    wav = wav / peak * 0.97
    return wav.astype("float32"), trimmed
            ...
            if builder is not None:
                ...
                try:
                    ...
                    STATE["tried"].append("%s.build_from_checkpoint: %s: %s"
                                          % (mod.__name__, type(e).__name__, str(e)[:1200]))
                    made = None
                if made is not None:
...
def run_convert(voice_wav, perf_wav, residual_from="performance", pitch_shift=0.0,
    ...
    wav, note = _as_audio(out)
    # The model may have degraded something rather than failing outright; say
    # so alongside the vocoder note instead of letting it pass unremarked.
    warn = getattr(model, "last_warning", "")
    if warn:
        note = (note + "; " + warn) if note else warn
    return wav, used, note
...
def run_generate(voice_wav, lyrics, melody_wav=None, steps=STEPS, pitch_shift=0.0):
    ...
    wav, note = _as_audio(out)
    return wav, "generation (%s)" % name, note
...
N_MELS = 100          # what the model card says the decoder emits


def looks_like_mel(arr):
    """Is this a mel spectrogram rather than a waveform?

    It matters because the two are not interchangeable and the difference is
    silent: flattening a (n_mels, frames) mel into a 1-D array interleaves
    frequency bins as if they were time samples, which plays as noise. The
    decoder in this model emits a 100-band mel, so this is the expected shape,
    not an edge case.
    """
    if arr.ndim != 2:
        return False
    a, b = arr.shape
    # One axis the size of the mel band count, the other much longer.
    return (a == N_MELS and b >= 8) or (b == N_MELS and a >= 8)


def mel_to_wave(mel):
    """Vocode a mel with Griffin-Lim, since no neural vocoder ships here.

    Griffin-Lim is phase-blind, so this sounds thinner and slightly metallic
    next to a trained vocoder. It is still recognisably the right voice singing
    the right notes, which is the whole point, and it is honest about what it is
    — the response says a fallback vocoder was used.
    """
    import librosa
    if mel.shape[0] != N_MELS:
        mel = mel.T
    power = np.exp(mel.astype("float64"))      # mel_spec() stored log-magnitude
    wav = librosa.feature.inverse.mel_to_audio(
        power, sr=SAMPLE_RATE, n_fft=1024, hop_length=256, n_iter=48)
    return np.asarray(wav, dtype="float32")


def _as_audio(out):
    """Whatever came back -> (mono float32 waveform, note about how).

    Batch and channel dimensions are squeezed before anything else. The previous
    version reshaped any multi-dimensional output into one long row, which is
    correct for (1, samples) and destroys a (1, n_mels, frames) mel.
    """
    note = ""
    if isinstance(out, dict):
        for k in ("audio", "wav", "waveform", "samples", "output", "mel", "mel_spectrogram"):
            if k in out:
                out = out[k]
                break
    ...
    while out.ndim > 1 and out.shape[0] == 1:      # drop batch/channel dims of 1
        out = out[0]

    if looks_like_mel(out):
        out = mel_to_wave(out)
        note = "mel output vocoded with Griffin-Lim (no neural vocoder in this build)"
    elif out.ndim > 1:
        # Genuinely multi-channel audio: average the channels, keeping time.
        time_axis = int(np.argmax(out.shape))
        out = out.mean(axis=1 - time_axis) if out.ndim == 2 else out.reshape(-1)

    out = np.asarray(out, dtype="float32").reshape(-1)
...
async def convert(voice: UploadFile = File(...),
    ...
    _guard()
    try:
        v, v_trim = load_audio(await voice.read(), label="voice clip")
        p, p_trim = load_audio(await performance.read(), label="performance clip")
        t0 = time.time()
        wav, used, notes = run_convert(v, p, residual_from, pitch_shift, steps, lyrics)
        headers = {"X-TriStream-Path": used,
                   "X-TriStream-Seconds": "%.1f" % (time.time() - t0),
                   "Content-Disposition": 'attachment; filename="tristream.wav"'}
        trims = ["%s %.0fs" % (n, t) for n, t in (("voice", v_trim), ("performance", p_trim)) if t]
        if trims:
            headers["X-TriStream-Trimmed"] = "first %.0fs used (%s)" % (MAX_SECONDS, ", ".join(trims))
        if notes:
            headers["X-TriStream-Note"] = notes
        return Response(content=to_wav_bytes(wav), media_type="audio/wav", headers=headers)
    except HTTPException:
        raise
    except ValueError as e:
        # Bad input, not a broken server. Say so with a 400 so the panel can
        # show the reason instead of "the server had an internal error".
...
async def generate(voice: UploadFile = File(...),
    ...
    try:
        ...
        if melody is not None:
            raw = await melody.read()
            if raw:
                m, m_trim = load_audio(raw, label="melody guide")
        t0 = time.time()
        wav, used, notes = run_generate(v, lyrics, m, steps, pitch_shift)
        headers = {"X-TriStream-Path": used,
                   "X-TriStream-Seconds": "%.1f" % (time.time() - t0),
                   "Content-Disposition": 'attachment; filename="tristream.wav"'}
        trims = ["%s %.0fs" % (n, t) for n, t in (("voice", v_trim), ("melody", m_trim)) if t]
        if trims:
            headers["X-TriStream-Trimmed"] = "first %.0fs used (%s)" % (MAX_SECONDS, ", ".join(trims))
        if notes:
            headers["X-TriStream-Note"] = notes
...
# ---------------------------------------------------------------------------
def start_tunnel():
    """Keep a quick tunnel up, and announce the address every time it changes.

    A quick tunnel is free and unguaranteed — cloudflared exits on its own often
    enough that treating it as permanent is wrong. The previous version read
    lines until it saw a URL, then drained the pipe until cloudflared exited, at
    which point the thread simply ended. The server carried on serving perfectly
    on localhost while its public address answered nothing, which from the
    outside is indistinguishable from the server having stopped — and a
    supervisor watching the server process sees nothing wrong, because nothing
    ...
    while True:
        ...
        try:
            ...
            log("!! could not start cloudflared: %s" % str(e)[:160])
            time.sleep(15)
            continue
        ...
        for line in proc.stdout:
            ...
            if m and m.group(0) != seen:
                ...
                log("")
                log("=" * 72)
                log(("TUNNEL URL: " if attempt == 1 else "NEW TUNNEL URL: ") + seen)
                if attempt > 1:
                    log("The previous address is dead. Paste this one into the Voice panel.")
                log("=" * 72)
                log("")
        ...
        log("!! the cloudflare tunnel exited (%s) after %.0fs — restarting it. "
            "The server itself is fine; only the public address was lost."
            % ("code %d" % code if code >= 0 else "signal %d" % -code, lived))
        # A tunnel that lasted a while is not a failing tunnel, so do not let a
        # long-lived one inherit the backoff of an early crash.
        if lived > 120:
            attempt = 1
...
# 4. Stay with it
#
# The cell used to print the URL and finish. That looks like the server stopped
# — and worse, a quick tunnel rotates its address, so when it does there is no
# longer anything running that could tell you the new one. Staying here prints
# the new address the moment it changes, shows a heartbeat so "still up" is
# visible rather than assumed, and keeps the notebook busy so the runtime is
# less likely to be reclaimed out from under it.
#
# Stopping this cell does not stop the server: it is detached on purpose and
# keeps serving. Re-running the cell adopts it.
# -----------------------------------------------------------------------------
if KEEP_ALIVE and url:
    import urllib.request as _u

    def _health():
        try:
            with _u.urlopen("http://127.0.0.1:%d/health" % PORT, timeout=4) as r:
                return json.loads(r.read())
        except Exception:
            return None

    print("\nWatching. The server keeps running if you stop this cell.\n")
    current, last_beat, quiet = url, 0.0, 0
    try:
        while True:
            time.sleep(2)
            ...
            if os.path.exists(URL_PATH):
                try:
                    fresh = open(URL_PATH).read().strip()
                except Exception:
                    fresh = ""
                if fresh and fresh != current:
                    current = fresh
                    print("\n" + "=" * 72)
                    print("  THE TUNNEL ADDRESS CHANGED. The old one is dead.")
                    print()
                    print("  URL:  " + current)
                    print()
                    print("  Paste this into the Voice panel and press Connect.")
                    print("=" * 72 + "\n")
            ...
            if h is None:
                quiet += 1
                print("[%s] no answer from the server (%d in a row) — check the log"
                      % (time.strftime("%H:%M:%S"), quiet))
                if _proc.poll() is not None:
                    print("The supervisor has exited. Last 20 log lines:\n")
                    print("".join(open(LOG_PATH).readlines()[-20:]))
                    break
                continue

            quiet = 0
            state = ("ready" if h.get("ok") else
                     "loading" if h.get("loading") else "not ready")
            extra = ""
            if h.get("restarts"):
                extra += " · %d server restart(s)" % h["restarts"]
            if h.get("tunnel_restarts"):
                extra += " · %d tunnel restart(s)" % h["tunnel_restarts"]
            print("[%s] %s · up %.0fs%s" % (time.strftime("%H:%M:%S"), state,
                                            h.get("uptime", 0), extra))
    except KeyboardInterrupt:
        print("\nStopped watching. The server is still running at " + current)
        print("Re-run this cell to watch it again, or check " + LOG_PATH)
